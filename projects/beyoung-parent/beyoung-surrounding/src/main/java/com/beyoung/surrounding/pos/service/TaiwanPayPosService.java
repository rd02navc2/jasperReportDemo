...
import com.beyoung.surrounding.dto.ResponseDTO;
import com.beyoung.surrounding.pos.entity.TAIWAN_TRANSACTION_LOG;
import com.beyoung.surrounding.pos.repository.TaiwanPayRepository;
...
public class TaiwanPayPosService {

    private final TaiwanPayRepository taiwanPayRepository;
    ...
    public ResponseBean processRefund(String orderID, String invoiceNO, String posID) {
		...
		try {		
			...
			taiwanPayRepository.findById(orderID)
			.orElseThrow(() ->
			...
			// 4. 查詢交易狀態
			String queryResult =
			purchaseService.taiwanpayQuery(
			      orderID, posID);
			...
			log.error("orderID -> {}, {} {}",
			  ...
			  bean.getMessage());
				...
				taiwanPayRepository.updateRefundInfo(
				orderID,
				refundTranId,
				transTime,
				invoiceNO);
    ...
    public ResponseDTO<?> queryTransactionStatus(String orderID) throws Exception {
        String queryResult = purchaseService.taiwanpayQuery(orderID, null);
