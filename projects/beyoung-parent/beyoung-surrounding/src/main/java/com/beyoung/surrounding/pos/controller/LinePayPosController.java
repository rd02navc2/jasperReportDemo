...
import com.beyoung.surrounding.pos.service.LinePayPosService;
...
@Slf4j 
@RestController
@RequestMapping("/Surrounding/rest/pos/LinePay")
@RequiredArgsConstructor
public class LinePayPosController {

	private final PurchaseService purchaseService; 
	private final LinePayPosService linePayPosService;
	...
	public ResponseBean purchase(
	    ...
	    try {
	        ...
	        // 1. 呼叫 LinePay 支付 API
	        String paymentResponse = purchaseService.linePayment(
	                orderID,
	                center,
	                counterID,
	                "比漾商品",
	     ...
	     // 1. 取得 transactionId
	        ...
	        // 2. 查詢 payment detail
	        String detailResponse = purchaseService.linePaymentDetail(transactionId);
	...
	public ResponseBean refund(
	    ...
	    try {
	     ...
	     // 1. 呼叫退款 API
	        String refundRes = purchaseService.linePayRefund(orderID);
	        ...
	        // 3. 查詢 detail
	        String detailRes =
	                purchaseService.linePaymentDetail(refundTransactionId);
	        ...
	        // 6. update DB
	        linePayPosService.updateRefundLog(
	                orderID,
	                refundTransactionId,
	                info.path("transactionType").asText(""),
	                sdf.parse(info.path("transactionDate").asText()),
